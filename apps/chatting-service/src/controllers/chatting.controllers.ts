...
export const getUserConversations = async (req: any, res: Response, next: NextFunction) => {
    try {
        ...
        const responseData = await Promise.all(conversations.map(async (group) => {
            //Get the sellerParticipant inside conversersion
            ...
            return {
                ...
                seller: {
                    ...
                    isOnline,
                    avatar: seller?.shop?.avatar || 'https://ik.imagekit.io/u4a8o785f/products/product-1760947357104_zM94niTsN.jpg?updatedAt=1760947360200'
                },
            ...
            conversations.map(async (group) => {
                ...
                return {
                    ...
                    user: {
                        ...
                        avatar:
                            user?.avatar ||
                            "https://ik.imagekit.io/u4a8o785f/products/product-1760947357104_zM94niTsN.jpg?updatedAt=1760947360200"
                    },
                    lastMessage: lastMessage?.content || "Say something to start a conversation",
                    lastMessageAt: lastMessage?.createdAt || group.createdAt,
                    unreadCount
...
export const fetchMessages = async (req: any, res: Response, next: NextFunction) => {
    try {
        ...
        return res.status(200).json({messages, seller: {
            id: seller?.id || null,
            name: seller?.shop?.name || "Unknown",
            isOnline,
            avatar: seller?.shop?.avatar || 'https://ik.imagekit.io/u4a8o785f/products/product-1760947357104_zM94niTsN.jpg?updatedAt=1760947360200'
        }, currentPage: page, hasMore: messages.length === pageSize });
...
export const fetchSellerMessages = async (req: any, res: Response, next: NextFunction) => {
    try {
        ...
        return res.status(200).json({
            ...
            name: user?.name || "Unknown",
            isOnline,
            avatar: user?.avatar || 'https://ik.imagekit.io/u4a8o785f/products/product-1760947357104_zM94niTsN.jpg?updatedAt=1760947360200'
        },
    currentPage: page,
