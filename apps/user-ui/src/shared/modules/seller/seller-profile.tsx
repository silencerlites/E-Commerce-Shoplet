...
import { Calendar, Clock, Facebook, FacebookIcon, Globe, Heart, Instagram, InstagramIcon, MapPin, Star, Twitter, Users, X, XIcon, YoutubeIcon } from "lucide-react"
    ...
    return (
        <div>
            <div className="relative w-full flex justify-center">
                <Image
                    src={shop?.coverBanner || "https://ik.imagekit.io/u4a8o785f/products/product-1755796313995_HRBen4Pab.jpg?updatedAt=1755796316237"}
                    alt="Seller Cover"
                ...
                <div className="bg-gray-200 p-6 rounded-lg shadow-lg flex-1">
                    <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
                        <div className="relative w-[100px] h-[100px] rounded-full border-4 border-slate-300 overflow-hidden">
                            <Image
                                src={shop?.avatar || "https://ik.imagekit.io/u4a8o785f/products/product-1755796313995_HRBen4Pab.jpg?updatedAt=1755796316237"}
                                alt="Seller Avatar"
                                layout="fill"
                                objectFit="cover" />
                        </div>
                        <div className="flex-1 w-full">
                            ...
                            <div className="flex items-center gap-3 mt-3 text-slate-700">
                                <Clock size={18} /> <span>{shop?.opening_hours || "Mon -Sat: 8:00 AM - 5:00 PM"}</span>
                            </div>

                            <div className="flex items-center gap-2 mt-3 text-slate-700">
                                <MapPin size={18} /> {" "} <span>{shop?.address || "No address provided"}</span>
                            </div>
                            <button className={`px-6 py-2 h-[40px] rounded-lg font-semibold flex items-center gap-2 transition ${isFollowing ? "bg-red-600 hover:bg-red-700" : "bg-blue-600 hover:bg-bluee-700"}`}
                                onClick={() => toggleFollowMutation.mutate()}
                                disabled={toggleFollowMutation.isPending}
                            >
                                <Heart size={18} /> {isFollowing ? "Unfollow" : "Follow"}
                            </button>
                    ...
                    <div className="bg-gray-200 p-6 rounded-lg shadow-lg w-full lg-w-[30%]">
                        ...
                        <div className="flex items-center gap-3 mt-3 text-slate-700">
                            <Calendar size={18} /> <span>Joined At: {new Date(shop?.createdAt!).toLocaleDateString()}</span>
                        </div>

                        {shop?.website && (
                            <div className="flex items-center gap-3 mt-3 text-slate-700">
                                <Globe size={18} /> <Link href={shop?.website} className="text-blue-600 hover:underline">{shop?.website}</Link>
                            </div>
                        )}
                                ...
                                <div className="flex gap-3 mt-2">
                                    {shop?.socialLinks.map((link: any, index: number) => (
                                        <a
                                            ...
                                            className="opacity-[.9]"
                                        >
                                            {link.type === "facebook" && <FacebookIcon size={24} />}
                                            {link.type === "X" && <XIcon size={24} />}
                                            {link.type === "Instagram" && <InstagramIcon size={24} />}
                                            {link.type === "Youtube" && <YoutubeIcon size={24} />}
                                        </a>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="flex border-b border-gray-300">
                        {TABS.map((tab) => (
                            <button key={tab} onClick={() => setActiveTab(tab)}
                                className={`py-3 px-6 text-lg font-semibold ${activeTab === tab ? "text-slate-800 border-b-2 border-blue-600" : "text-gray-600"} transition`}
                            >
                                {tab}
                            </button>
                        ))}
