    ...
    const { data, isLoading } = useQuery({
       ...
       const columns: ColumnDef<any>[] = [
        ...
        },
        {
            accessorKey: "total",
            header: "Total",
            cell: (info: any) => `Php ${info.getValue()?.toFixed(2)}`,
        },
        ...
        },
        {
            ...
            header: "Actions",
            cell: ({ row }) => (
                <button onClick={() => router.push(`/order/${row.original.id}`)} className="text-blue-600 hover:underline text-xs items-center gap-1">Track Order<ArrowUpRight className='w-3 h-3'/></button>
            )
        },
    ];
