...
const ProductCard = ({ product, isEvent }: { product: any; isEvent?: boolean }) => {
    ...
    useEffect(() => {
        if (isEvent && product?.ending_date) {
            const interval = setInterval(() => {
                ...
                setTimeLeft(`${days}d ${hours}h ${minutes}m left with this price ${product?.price}`);
    ...
    return (
        <div className='w-full min-h-[350px] h-max bg-white rounded-lg relative'>
            ...
            {isEvent && timeLeft && (
                <div className='mt-2'>
                    <span className='inline-block text-xs bg-orange-100 text-orange-600 font-medium mr-2 px-2.5 py-0.5 rounded'>
                        {timeLeft}
                    </span>
                </div>
            )}
            ...
            <div className='absolute z-10 flex flex-col gap-3 right-3 top-10'>
                <div className='bg-white rounded-full p-[6px] shadow-md'>
                    <Heart className='cursor-pointer hover:scale-110 transition' size={22} fill={isWishlisted ? 'red' : 'transparent'} stroke={isWishlisted ? 'red' : '#4b5563'} 
