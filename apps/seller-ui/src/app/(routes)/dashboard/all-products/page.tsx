...
import { Search, Pencil, Trash, Eye, Plus, BarChart, Star, ChevronRight, ShoppingBag } from 'lucide-react'
import Link from 'next/link'
import axiosInstance from 'apps/seller-ui/src/utils/axiosInstance'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import Image from 'next/image'
import DeleteConfirmation from 'apps/seller-ui/src/shared/components/modals/delete.confirmation'
        ...
        () => [
            {
                ...
                cell: ({ row }: any) => {
                    ...
                    return (
                        ...
                        <Image  
                            src={row.original.images[0]?.url || <ShoppingBag size={48} />}
                            alt={row.original.images[0]?.url || <ShoppingBag size={48} />}
                        width={48}
                        height={48}
            ...
            },
            {
                ...
                header: 'Product Name',
                cell: ({ row }: any) => {
                    const truncatedTitle = row.original.title.length > 25 ? `${row.original.title.slice(0, 25)}...` : row.original.title;
                    return (
                        <Link href={`${process.env.NEXT_PUBLIC_USER_UI_LINK}/product/${row.original._id}`} className='text-blue-400 hover:underline'>{truncatedTitle}</Link>
                    )
                }
            },
            {
                ...
                header: 'Price',
                cell: ({ row }: any) => {
                    return <span>${row.original.regular_price}</span>;
                }
            },
            {
                accessorKey: 'stock',
                header: 'Stocks',
                cell: ({ row }: any) => {
                   return <span className={row.original.stock < 10 ? 'text-red-500' : 'text-white'}>
    ...
    return (
        <div className='w-full min-h-screen p-8'>
            ...
            <div className='mb-4 flex items-center bg-gray-900 p-2 rounded flex-1'>
                ...
                <input type="text" placeholder='Search products...' className='w-full bg-transparent outline-none text-white' value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} />
            </div>
