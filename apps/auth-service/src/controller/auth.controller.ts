...
import { checkOtpRestrictions, handleForgotPassword, sendOtp, trackOtpRequest, validationRegistrationData, verifyForgotPasswordOtp, verifyOtp } from "../utils/auth.helper";
...
export const resetUserPassword = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!email || !newPassword) return next(new ValidationError("All fields are required"));
        ...
        if (isSamePassword) return next(new ValidationError("New password must be different from the existing password"));
